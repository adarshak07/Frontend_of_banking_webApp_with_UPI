...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (oauthError) {
      if (oauthError === 'sso_disabled') {
        setError('SSO is currently disabled. Please use email and password to sign in.');
      } else if (oauthError === 'oauth_error') {
        setError('OAuth authentication failed. Please try again or use email and password.');
      } else if (oauthError === 'oauth_failed') {
        setError('OAuth authentication failed. Please try again or use email and password.');
      } else if (oauthError === 'oauth_session_expired') {
        setError('OAuth session expired. Please try signing in again.');
      } else if (oauthError === 'oauth_access_denied') {
        setError('Access denied. Please grant necessary permissions and try again.');
      } else if (oauthError === 'oauth_invalid_grant') {
        setError('Invalid OAuth request. Please try again.');
      }
    }
  }, [searchParams]);
  ...
  const checkSsoStatus = async () => {
    try {
      const response = await api.get('http://localhost:8082/auth/sso-status');
  ...
  return (
    <div className="card" style={{ maxWidth: 600, margin: "40px auto", padding: "2rem" }}>
      <h2 className="title" style={{ fontSize: "2rem", marginBottom: "0.5rem" }}>Welcome back</h2>
      <p className="muted" style={{ fontSize: "1.1rem", marginBottom: "2rem" }}>Log in to continue to your banking dashboard</p>
      ...
      <div style={{ marginTop: "2rem", textAlign: "center" }}>
        <div className="muted" style={{ fontSize: "1rem" }}>
          Don't have an account? <Link to="/register" style={{ color: "#3b82f6", textDecoration: "none", fontWeight: "500" }}>Create one here</Link>
        </div>
      </div>
    </div>
  );
